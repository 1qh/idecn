...
if curl -sf "http://localhost:$PORT/r/idecn.json" >/dev/null 2>&1; then
	...
	STARTED_SERVER=true
	sleep 5
	curl -sf "http://localhost:$PORT/r/idecn.json" >/dev/null || {
		echo "x Dev server failed"
		exit 1
	}
fi
...
find app \( -name '*.ts' -o -name '*.tsx' \) -exec sed -i.bak "s|from 'idecn'|from '@/components/ui/idecn'|g" {} +
rm -f app/*.bak
